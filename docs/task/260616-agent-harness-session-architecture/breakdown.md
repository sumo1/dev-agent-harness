...
| S8 | 收尾串行 | 必须在核心路径实现后验证 UI/API/DB/log/trace 是否闭环，不能和功能开发混跑 |
...
把 Goal、Issue、Assistant、OpenClaw/Lobster 四条路径按桌面端真实操作跑一遍，并用 UI、API、DB、server/daemon log、computer-use trace 做证据闭环。

产出文件：[`plan/step-8-e2e-verification.md`](./plan/step-8-e2e-verification.md)、[`e2e-verification-cases.md`](./e2e-verification-cases.md)
...
- 每条关键运行都能从 UI transcript、API、DB、server/daemon log 和 computer-use trace 查到同一条证据链。
- 桌面端 E2E 必须使用候选 desktop + `computer-use-harness`，不能用浏览器验证替代。
