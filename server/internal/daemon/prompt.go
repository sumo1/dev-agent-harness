...
func buildGoalPlanningPrompt(task Task) string {
	...
	b.WriteString("Each node: {\"seq\": <int>, \"title\": \"...\", \"spec\": \"...\", \"assignee_agent_id\": \"<uuid>\", \"depends_on\": [<seq>...], \"kind\": \"execute\"|\"verify\"}. `depends_on` lists the `seq` numbers that must finish first; an empty list starts the node in the first wave. Omit `kind` for execute. Assign by copying a role's UUID from its roster entry.\n\n")

	b.WriteString("Example — COHERENT COGNITIVE WORK (\"give an architecture overview and a pros/cons analysis of this codebase\"). One analyst reads, understands, and writes it; one different-model reviewer checks it. No fan-out, no merge node:\n")
	b.WriteString("  [{\"seq\":1,\"title\":\"Architecture overview and pros/cons\",\"spec\":\"Read the codebase, build one coherent mental model, and write the architecture overview plus a pros/cons analysis. Acceptance: covers the layering and major components, the main data/control flow, and states concrete trade-offs with where each one bites.\",\"assignee_agent_id\":\"<analyst-uuid>\",\"depends_on\":[]},{\"seq\":2,\"title\":\"Review the overview\",\"spec\":\"Adversarially review the architecture overview for accuracy, gaps, and unsupported claims; return a pass/reject verdict with specifics.\",\"assignee_agent_id\":\"<reviewer-uuid>\",\"depends_on\":[1],\"kind\":\"verify\"}]\n\n")
	b.WriteString("Example — INDEPENDENT CONSTRUCTION (a feature with separable build units). The first wave builds three units in parallel against a shared contract, a different-model review fans in over them, then one node wires them together:\n")
